...
- **Change specs** (`/specs/changes`) are historical — mark `Implemented`/`Superseded`, never rewrite. Statuses: `Proposed` → `Accepted` → `In Progress` → `Implemented` (or `Superseded`). **`Implemented` means "merged via this PR": flip the status (Status line + a history row) inside the implementation PR, not as a separate post-merge edit that is easy to forget.** `## Status` must always equal the latest `## Status history` row — `scripts/spec-status-guard.mjs` enforces this (plus valid enum, legal transitions, monotonic dates) in `pre-commit`, `npm run check`, and CI, and `scripts/pr-spec-reference.mjs` blocks a PR that lands code for a `Proposed`/missing spec (spec 0031). Required sections: see `/templates/change-spec-template.md`. Every spec states its test strategy.
