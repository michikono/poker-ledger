...
6. **Debounced, visibility-aware banner (no layout thrash).** The connection **light** reflects state immediately — a brief blip turns the dot red and back with no layout change. The **banner** only appears after the connection has been continuously non-live for a short delay (`BANNER_SHOW_DELAY_MS`, ~10s) **of visible time**, and the delay **resets whenever the tab becomes visible**. The banner is hidden while the tab is hidden. So:
   - A momentary blip while you're looking never renders the banner.
   - **Lock/unlock case (the one to prevent):** you're connected, lock the phone, and unlock after a long time. The tab was hidden the whole time, so the banner never armed; on unlock the delay restarts from zero and the connection resumes within ~1s — **the banner never flashes.** You just see the light briefly settle to green.
   - Only a disconnect that persists for the full delay *after* the tab is visible (e.g. a real, sustained network/Firebase outage, or the 10-minute idle-out reached while you're actively looking) surfaces the hard banner message.
   When the connection returns to live, the banner hides immediately.
...
- **`StaleSyncBanner` (extended, fake timers + visibility stub):**
  - Does **not** render immediately when status goes non-live; renders only after `BANNER_SHOW_DELAY_MS` elapses while still non-live and visible.
  - A return to live before the delay means it never renders (and hides immediately if already shown).
  - **Stays hidden while the tab is hidden**, and the delay **restarts on `visibilitychange → visible`** — the lock/unlock reconnect case: start non-live + hidden, become visible, then go live within the delay → the banner never renders.
  - Renders a "Refresh now" control that calls the context `reconnect`.
...
- [ ] The connection light reflects state immediately (red on a blip) with no layout shift; the banner only appears after a sustained non-live period (`BANNER_SHOW_DELAY_MS`) of visible time and hides immediately on recovery — a quick blip never renders the banner.
- [ ] Lock/unlock never flashes the banner: hidden-while-locked never arms it, and the delay resets on tab-visible, so a resume that reconnects within the delay shows no banner.
...
- **Banner debounce (visibility-aware).** In `StaleSyncBanner`, gate visibility on a delayed flag via an effect keyed on `isLive(status)`:
  - When live → clear any timer, `show = false`, return.
  - When non-live → an `arm()` that clears the old timer and, **only if `document.visibilityState === "visible"`**, starts a `BANNER_SHOW_DELAY_MS` timer that sets `show = true`.
  - Add a `visibilitychange` listener: on → visible, `show = false` then `arm()` (restart the delay); on → hidden, `show = false` and clear the timer.
  - Call `arm()` once on entry. Return `null` until `show`.
  This makes a locked (hidden) tab never arm the banner, and a resume-on-unlock restart the delay — so a quick reconnect shows nothing. The message text still reads the current `status` at render, so a paused-idle vs offline change while shown updates copy without re-timing. The **light is not debounced** — it renders `status` directly.
